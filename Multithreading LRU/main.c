...
int simulation_length = 5; // default is 30 seconds
...
// two additional unit test

void self_tests1() {
    int rv;
    int round = 0;

    // reference 64 times, with each time reference a random key that is less than 64
    while(round < 64) {
        int i = rand() % 64;
        rv = reference(i);
        if (!rv) die ("Failed to reference key %d\n", i);
...
void self_tests2() {
    ...
    // should hang as the count is greater than the HIGH_WATER_MARK
    rv = reference(97);
    // should not print
    print();
}
...
void self_tests3() {
    ...
    // should hang as the count is smaller than 32
    clean(1);
    // should not print
    print();
}
